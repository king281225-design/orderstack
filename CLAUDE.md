...
**Not yet done, and can't be from this chat:** no real Postgres has been provisioned, so no migration has ever actually been run against a live database — the schema/queries are verified by typecheck + build only, not by a real end-to-end run. See "Running locally / deploying" below for exactly what the user needs to do to unblock that. Days 10–12 (loading a real restaurant's real menu, phone bug-bash, production deploy) are inherently the user's to do once a database exists.
...
1. **Provision Postgres** — create a Neon or Supabase project, copy its connection string into `DATABASE_URL` in `.env` (copy `.env.example` → `.env` first).
...
3. Run `npm install` (already done in this environment) then:
   - `npx prisma migrate dev --name init` — creates the tables. This is the step that has never actually run yet; it's the first thing to try against a real `DATABASE_URL`.
   - `npm run db:seed` — bootstraps the first `SUPER_ADMIN` from `SUPER_ADMIN_EMAIL` / `SUPER_ADMIN_PASSWORD` in `.env`.
4. `npm run dev`, sign in at `/login` with the super-admin credentials, add a restaurant, sign in as its owner, build the menu, then open `/r/<slug>` to place a test order.
...
None of this has been run yet in this environment — no Postgres instance is reachable from here, so `prisma migrate dev`, seeding, and an actual order round-trip are all unverified beyond typecheck + build. Do this first pass before Day 10 of the plan.
